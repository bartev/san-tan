...
import swing._
import swing.event.{ButtonClicked, TableRowsSelected, TableColumnsSelected}
...
object TableSelection extends SimpleSwingApplication {
  ...
  lazy val ui = new BoxPanel(Orientation.Vertical) {
    ...
    val output = new TextArea(4, 40) {editable = false}
    ...
    reactions += {
      ...
      case ButtonClicked(`rowSelection`) =>
        if (rowSelection.selected) table.selection.elementMode = Table.ElementMode.Row
      case ButtonClicked(`columnSelection`) =>
        if (columnSelection.selected) table.selection.elementMode = Table.ElementMode.Column
      case ButtonClicked(`cellSelection`) =>
        if (cellSelection.selected) table.selection.elementMode = Table.ElementMode.Cell
