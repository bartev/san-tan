...
if __name__ == "__main__":
    if len(sys.argv) > 3:
        print("Usage: gen_mnm_dataset entries [fname]", file=sys.stderr)
        sys.exit(-1)
    ...
    dataset_fn = sys.argv[2] if len(sys.argv) > 2 else "mnm_datasets.csv"

    with open(dataset_fn, mode='w') as dataset_file:
        dataset_writer = csv.writer(
            dataset_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL
        )
        dataset_writer.writerow(fieldnames)
        for i in range(1, entries):
            dataset_writer.writerow(
                [
                    get_random_choice(states),
